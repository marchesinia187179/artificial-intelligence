# Orange or Grapefruit Classifier using Linear Discriminant Analysis

This project implements a Linear Discriminant Analysis (LDA) classifier from scratch to distinguish between oranges and grapefruits based on physical characteristics such as diameter, weight, and color components (red, green, blue).

## Overview

The `main.py` file contains a complete implementation of a Linear Discriminant Analysis classifier. It loads data from `data.csv`, trains the model, makes predictions, and evaluates accuracy. The classifier assumes a shared covariance matrix across classes and uses maximum likelihood estimation for the parameters.
...
### Linear_Discriminant_Analysis Class

#### `__init__`
Initializes the classifier with placeholders for classes, number of classes, priors, means, and covariance matrix.

#### `fit(X_train, y_train)`
Trains the model by:
- Converting data to numpy arrays.
- Identifying unique classes.
- Calculating mean for each feature per class and prior probabilities.
- Computing the pooled covariance matrix across all classes.
- Printing the computed statistics for each class.

#### `predict(X_test)`
Makes predictions for test data by:
- For each test sample, computing the posterior probability for each class using the log of priors and log-likelihoods.
- Selecting the class with the highest posterior.

#### `_log_likelihood(x_test, idx)`
Computes the log-likelihood for a feature vector given the class mean and shared covariance matrix using the Mahalanobis distance.
...
- Show mean and prior for each class during training.
- Output the pooled covariance matrix.
