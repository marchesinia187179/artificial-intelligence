...
class Eigenfaces:
    ...
    def fit(self, X: np.ndarray, verbose: bool = False):
        ...
        if verbose:
            # show mean face
            plt.imshow(self._x_mean.reshape(112, 92), cmap='gray')
            plt.title('mean face')
        ...
        principal_eigval, principal_eigvec = eigval[:self.n_components], eigvec[:, :self.n_components]

        # retrieve original eigenvec
        self._ef = X_norm.T @ principal_eigvec

        # normalize the retrieved eigenvectors to have unit length
        self._ef = self._ef / (principal_eigval ** 0.5)
